...
describe("TEST REPOREDUCER", () => {
  ...
  test("TEST ISSUETOREPOADDED WITH FILLED STATE", () => {
    ...
    expect(reposReducer(initialState, action).repos[0].openIssues[0].id).toBe(
      CLOSED_ISSUE.id
    );
  ...
  test("TEST ISSUEORDERCHANGED TO INSERTING AFTER ELEMENT", () => {
    ...
    expect(reposReducer(initialState, action).repos[0].closedIssues[1].id).toBe(
      OPENED_ISSUE.id
    );
  });
  ...
  test("TEST ISSUEORDERCHANGED TO INSERTING BEFORE ELEMENT", () => {
    ...
    expect(reposReducer(initialState, action).repos[0].closedIssues[0].id).toBe(
      OPENED_ISSUE.id
    );
  });
